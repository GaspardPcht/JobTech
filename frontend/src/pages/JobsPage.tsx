...
import { filterTechOffers } from '../services/offerService';
...
const JobsPage: React.FC = () => {
  ...
  // Filtrer les offres pour ne garder que celles liées à la tech
  useEffect(() => {
    if (allOffers.length > 0) {
      const techOffers = filterTechOffers(allOffers);
      setFilteredOffers(techOffers);
    } else {
      setFilteredOffers([]);
    }
  }, [allOffers]);
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      ...
      <p className="text-gray-600 mb-8">
        Découvrez les dernières offres d'emploi dans le domaine de la tech,
        mises à jour en temps réel depuis Adzuna et Pôle Emploi.
      </p>
      ...
      {!isLoading && !error && filteredOffers.length === 0 && (
        <div
          ...
          role="alert"
        >
          <p className="font-bold">Aucune offre tech trouvée</p>
          <p>
      ...
      {!isLoading && !error && filteredOffers.length > 0 && (
        <>
          <p className="text-gray-600 mb-4">
            {filteredOffers.length} offres tech trouvées
          </p>
